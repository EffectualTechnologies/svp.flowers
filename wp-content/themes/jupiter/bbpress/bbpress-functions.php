...
class BBP_Default extends BBP_Theme_Compat {
	...
	private function setup_globals() {
		$bbp           = bbpress();
		$this->id      = 'default';
		$this->name    = __( 'bbPress Default', 'mk_framework' );
	...
	public function localize_topic_script() {
		...
		wp_localize_script( 'bbpress-topic', 'bbpTopicJS', array(
			...
			'generic_ajax_error' => __( 'Something went wrong. Refresh your browser and try again.', 'mk_framework' ),
	...
	public function ajax_favorite() {
		...
		if ( ! bbp_is_favorites_active() ) {
			bbp_ajax_response( false, __( 'Favorites are no longer active.', 'mk_framework' ), 300 );
		}

		// Bail if user is not logged in
		if ( !is_user_logged_in() ) {
			bbp_ajax_response( false, __( 'Please login to make this topic a favorite.', 'mk_framework' ), 301 );
		}
		...
		if ( !current_user_can( 'edit_user', $user_id ) ) {
			bbp_ajax_response( false, __( 'You do not have permission to do this.', 'mk_framework' ), 302 );
		}
		...
		if ( empty( $topic ) ) {
			bbp_ajax_response( false, __( 'The topic could not be found.', 'mk_framework' ), 303 );
		}

		// Bail if user did not take this action
		if ( !isset( $_POST['nonce'] ) || !wp_verify_nonce( $_POST['nonce'], 'toggle-favorite_' . $topic->ID ) ) {
			bbp_ajax_response( false, __( 'Are you sure you meant to do that?', 'mk_framework' ), 304 );
		}
		...
		if ( empty( $status ) ) {
			bbp_ajax_response( false, __( 'The request was unsuccessful. Please try again.', 'mk_framework' ), 305 );
		}
	...
	public function ajax_subscription() {
		...
		if ( !bbp_is_subscriptions_active() ) {
			bbp_ajax_response( false, __( 'Subscriptions are no longer active.', 'mk_framework' ), 300 );
		}

		// Bail if user is not logged in
		if ( !is_user_logged_in() ) {
			bbp_ajax_response( false, __( 'Please login to subscribe to this topic.', 'mk_framework' ), 301 );
		}
