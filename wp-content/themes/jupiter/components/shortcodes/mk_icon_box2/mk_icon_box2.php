...
if ($icon_type == 'icon'){
	$app_styles = '
		#mk-icon-box-'.$id.' .mk-box-icon-2-icon {
		    font-size:'.$icon_size.'px; '.($icon_color ? ('color:'.$icon_color.'!important;') : '').($icon_background_color ? ('background-color:'.$icon_background_color.';') : '').($icon_border_color ? ('border:1px solid '.$icon_border_color.';') : '').'
		}
	...
	$output .= !empty( $read_more_url ) ? '<a href="'.$read_more_url.'" target="_blank">' : '';
