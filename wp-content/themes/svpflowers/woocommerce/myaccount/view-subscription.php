...
//				echo '<pre>';print_r($svp_cron_subscr_restart_time);echo '</pre>';
				?>
				<?php esc_html_e( 'Actions', 'woocommerce-subscriptions' ); ?></td>
			<td>
				<?php
				$is_suspend_btn = 0;
				?>
		...
		if( $is_suspend_btn == 1 ) :
			?>
			<tr>
				<td style="display: none;"><?php _e( 'Pause For: ' ) ; ?></td>
				<td style="display: none;">
					...
					<select name="suspending_unit" id="suspending_unit" style="display: inline; min-width: 140px ">
						<!-- <option value="day"><?php _e( 'Day(s)' ); ?></option> -->
						<option value="week"><?php _e( 'Week(s)' ); ?></option>
						<option value="month"><?php _e( 'Month(s)' ); ?></option>
					</select>
