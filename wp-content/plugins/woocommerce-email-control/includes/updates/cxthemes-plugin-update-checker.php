...
class CX_Email_Control_Plugin_Update_Checker {
	...
	public function __construct( $pluginFile, $slug ) {
		...
		// Debugging:
		/*if ( 0 === strpos( $_SERVER['HTTP_HOST'], 'http://localhost' ) )
			$metadataUrl = 'http://localhost/cxthemes/update-server/'; // Local
		else
			$metadataUrl = 'http://updates.cxthemes.com/'; // Live*/
			...
			if ( isset( $_GET['plugin'] ) && $this->slug == $_GET['plugin'] ) {
				wp_localize_script(
					...
					array(
						...
						'plugin_register_url' => esc_url( admin_url( 'plugins.php?' . WC_EMAIL_CONTROL_PLUGIN_FOLDERNAME . '-purchase-code' ) ),
					)
				);
			}
		}
	...
	function show_purchase_code_interface() {
		...
		if ( 'plugins.php' == $pagenow && isset( $_GET[WC_EMAIL_CONTROL_PLUGIN_FOLDERNAME.'-purchase-code'] ) ) {
			
			// Get submitted purchase code.
			$submitted_code = isset( $_POST['ec-plugin-update-code'] ) ? trim( $_POST['ec-plugin-update-code'] ) : '' ;
			
			// Validation, on form submit.
			
			$notification = FALSE; // Control var.
			
			if ( isset( $_POST['ec-plugin-update-code-action'] ) && 'submit' == $_POST['ec-plugin-update-code-action'] ) {
			...
			<div class="cx-plugin-update-enter-code notice notice-success is-dismissibleZZ">
				<form method="post">
					...
					<input type="hidden" name="ec-plugin-update-code-action" value="submit">
					...
					<div class="ec-plugin-update-row">
						...
						<input type="text" name="ec-plugin-update-code" value="<?php echo esc_attr( ( $submitted_code ? $submitted_code : get_option( "cx_{$this->slug}_envato_purchase_code" ) ) ); ?>" />&nbsp;
	...
	public function plugin_row_meta( $links, $file ) {
		...
		if ( $file == WC_EMAIL_CONTROL_PLUGIN_BASENAME ) {
			...
			if ( ! $purchase_code ) {
				...
				$url = esc_url( admin_url( 'plugins.php?' . WC_EMAIL_CONTROL_PLUGIN_FOLDERNAME . '-purchase-code' ) );
				...
				$row_meta = array(
					'support' => '<a class="cx-plugin-update-row-meta-change-purchase-code" href="' . esc_url( '?'.WC_EMAIL_CONTROL_PLUGIN_FOLDERNAME.'-purchase-code' ) . '" title="' . esc_attr( __( '', 'email-control' ) ) . '">' . __( 'Edit registration', 'email-control' ) . '</a>',
				);
				$links = array_merge( $links, $row_meta );
			}
		}
