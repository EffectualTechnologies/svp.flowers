...
function elfsight_yottie_widgets_get_table_name() {
  global $wpdb;

  return $wpdb->prefix . str_replace('-', '_', ELFSIGHT_YOTTIE_SLUG) . '_widgets';
}

function wp_elfsight_yottie_widgets_table_exists() {
  global $wpdb;

  $table_name = elfsight_yottie_widgets_get_table_name();
...
function wp_elfsight_yottie_widgets_upgrade() {
	if (wp_elfsight_yottie_widgets_table_exists()) {
		return;
	}

	elfsight_yottie_widgets_create_table();
}
...
function elfsight_yottie_widgets_create_table() {
  $table_name = elfsight_yottie_widgets_get_table_name();
...
register_activation_hook(ELFSIGHT_YOTTIE_FILE, 'wp_elfsight_yottie_widgets_upgrade');
...
function elfsight_yottie_widgets_api_get_list(&$result) {
  global $wpdb;
...
function elfsight_yottie_widgets_api_post_add(&$result) {
  global $wpdb;
  ...
  if ($invalid_fields) {
    $result['status'] = false;
    $result['error'] = __('Incoming data is invalid.', ELFSIGHT_YOTTIE_TEXTDOMAIN);
    ...
    if (!$status) {
      $result['error'] = __('An MySQL error occurred while adding new widget.', ELFSIGHT_YOTTIE_TEXTDOMAIN);

    } else if (get_option('elfsight_yottie_widgets_clogged') !== 'true') {
      update_option('elfsight_yottie_widgets_clogged', 'true');
    }
  }
}

function elfsight_yottie_widgets_api_post_remove(&$result) {
  global $wpdb;
  ...
  if (!$id) {
    $result['status'] = false;
    $result['error'] = __('Parameter "id" is required.', ELFSIGHT_YOTTIE_TEXTDOMAIN);
  ...
  if (!$status) {
    $result['error'] = __('Widget with specified id doesnt exist.', ELFSIGHT_YOTTIE_TEXTDOMAIN);
  }
}

function elfsight_yottie_widgets_api_post_restore(&$result) {
  global $wpdb;
...
function elfsight_yottie_widgets_api_post_update(&$result) {
  global $wpdb;
...
function elfsight_yottie_widgets_api() {
  $result = array();

  $method = strtolower($_SERVER['REQUEST_METHOD']);
  $endpoint = !empty($_REQUEST['endpoint']) ? $_REQUEST['endpoint'] : '';
  $endpoint_handler_name = sprintf('elfsight_yottie_widgets_api_%s_%s', $method, $endpoint);
...
add_action('wp_ajax_elfsight_yottie_widgets_api', 'elfsight_yottie_widgets_api');
