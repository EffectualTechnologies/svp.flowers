...
function smart_coupon_deactivate() {
	if ( get_option( 'smart_coupon_email_subject' ) !== FALSE ) {
	...
	if ( ! class_exists( 'WC_Smart_Coupons' ) ) {
		...
		class WC_Smart_Coupons {
			...
			public function __call( $function_name, $arguments = array() ) {
				...
				if ( ! is_callable( 'Smart_Coupons_WC_Compatibility_2_4', $function_name ) ) return;

				if ( ! empty( $arguments ) ) {
					return call_user_func_array( 'Smart_Coupons_WC_Compatibility_2_4::'.$function_name, $arguments );
				} else {
					return call_user_func( 'Smart_Coupons_WC_Compatibility_2_4::'.$function_name );
				}
			...
			public function show_available_coupons( $available_coupons_heading = '', $page = 'checkout' ) {
				...
				$global_coupons = $wpdb->get_results("SELECT * 
													FROM {$wpdb->prefix}posts
													WHERE FIND_IN_SET (ID, (SELECT option_value FROM {$wpdb->prefix}options WHERE option_name = 'sc_display_global_coupons')) > 0
													GROUP BY ID 
